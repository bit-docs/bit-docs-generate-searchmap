var assert = require('assert'),
	...
	searchMapHash = require('./generators/search-map-hash'),
...
var docMap = {
		  "can-core": {
		    ...
		    "description": "The best, most hardened and generally useful libraries in __CanJS__.\n",
		    "name": "can-core",
		    "title": "Core",
		    "type": "page",
		    "parent": "canjs",
...
describe("bitDocs.generators.searchMap",function(){
	...
	it("(bitDocs.generators.searchMap.searchMap) Parses markdown in the description", function(done){
		var descriptionShouldEqual = '<p>The best, most hardened and generally useful libraries in <strong>CanJS</strong>.</p>';
	...
	it("(bitDocs.generators.searchMap.searchMapHash) Generates a searchMapHash file", function(done){
		docMapPromise.then(function(docMap){
			searchMapHash(docMap, siteConfig).then(function(){
				if(!fs.existsSync(path.join(__dirname,"test_tmp","searchMapHash.json"))) {
					done(new Error("searchMapHash.json does not exist"));
				} else{
	...
	it("(bitDocs.generators.searchMap.searchMapHash) Generates a new hash when the searchMap changes", function(done){
		docMapPromise.then(function(docMap){
			searchMap(docMap, siteConfig).then(function(searchMapResult){
				searchMapHash(searchMapResult, siteConfig).then(function(firstHash){
					if(!fs.existsSync(path.join(__dirname,"test_tmp","searchMapHash.json"))) {
						done(new Error("searchMapHash.json does not exist (1)"));
					} else{
						searchMapResult.description = "test " + searchMapResult.description;
						searchMapHash(searchMapResult, siteConfig).then(function(secondHash){
							if(!fs.existsSync(path.join(__dirname,"test_tmp","searchMapHash.json"))) {
								done(new Error("searchMapHash.json does not exist (2)"));
	...
	it("(bitDocs.generators.searchMap) Generates searchMap and searchMapHash files", function(done){
		generator(docMapPromise, siteConfig).then(function(){
			if(!fs.existsSync(path.join(__dirname,"test_tmp","searchMap.json"))) {
				done(new Error("searchMapHash.json does not exist"));
